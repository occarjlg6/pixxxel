...
impl<T> Canvas<T> {
    ...
    pub fn set(&mut self, x: usize, y: usize, value: T) -> Result<(), CanvasError> {
        ...
        // I think this is acceptable because we've guaranteed a valid index.
    ...
    pub fn export_png(&self, path: &Path) -> Result<(), image::ImageError> {
        ...
        for py in 0..self.height {
            for px in 0..self.width {
                // Cells are 0x00RRGGBB, so the big-endian bytes are [unused, r, g, b].
                let [_, r, g, b] = self.cells[px + py * self.width].to_be_bytes();
